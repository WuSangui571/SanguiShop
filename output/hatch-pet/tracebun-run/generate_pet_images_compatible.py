#!/usr/bin/env python3
"""Compatible image generation fallback for a hatch-pet run.

This is a workspace-local copy of the hatch-pet secondary fallback with one
important difference: the image API base URL is configurable so we can target
...
def complete_job(job: dict[str, object], output_path: Path, source_url: str) -> None:
    job["status"] = "complete"
    ...
    job["secondary_fallback_base_url"] = source_url
    for key in [
        "last_error",
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="OpenAI-compatible image API base URL, e.g. https://api.example.com/v1",
    )
    ...
    for job in jobs:
        ...
        print(f"Generating {job_id} with compatible secondary fallback via {base_url}")
        ...
        complete_job(job, output_image, base_url)
        if job_id == "base":
