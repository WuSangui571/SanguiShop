...
- `POST /api/users/ops/session/refresh` is JWT-protected and should only mint a new token for principals that still resolve to configured admin-session permission bindings or legacy rollback admins.
