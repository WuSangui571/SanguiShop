...
- Good: the manual `.github/workflows/compensation-ops-audit.yml` `workflow_dispatch` workflow uses Node 24-compatible official actions, declares `permissions: contents: read`, runs `./scripts/verify-compensation-ops-audit.ps1 -Service <all|order|payment>` on `ubuntu-latest` with `pwsh`, and the run log confirms the target test class or classes executed.
